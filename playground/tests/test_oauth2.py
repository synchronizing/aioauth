...
async def test_password_grant_type(client: TestClient, client_record: ClientTable):
    ...
    response = await client.post("/token", form=data, headers=headers)
    assert "access_token" in response.json()


@pytest.mark.asyncio
async def test_implicit_flow(client: TestClient, client_record: ClientTable):
    query = {
    ...
    response = await client.get("/authorize", query_string=query)
    ...
    response = await client.post(
        "/authorize", query_string=query, form=data, allow_redirects=False
    )
    assert response.status_code == status.HTTP_302_FOUND
    redirect_uri = urlparse(response.headers["location"])
    fragment = dict(parse_qsl(redirect_uri.fragment))
    assert "access_token" in fragment


@pytest.mark.asyncio
async def test_authorization_code_flow(
    client: TestClient, client_record: ClientTable, app
):
    # Check get method
    query = {
        "client_id": client_record.client_id,
    ...
    response = await client.get("/authorize", query_string=query, allow_redirects=False)
    assert response.status_code == status.HTTP_200_OK

    # Get authorization code
    response = await client.post("/authorize", query_string=query)
