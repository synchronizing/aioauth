...
from async_oauth2_provider.response_type import (ResponseTypeAuthorizationCode,
                                                 ResponseTypeToken)
from async_oauth2_provider.types import EndpointType, GrantType, ResponseType
from fastapi_oauth2.config import settings
from fastapi_oauth2.tables import (AuthorizationCodeTable, ClientTable,
                                   TokenTable)
...
class PostgreSQL(DBBase):
    ...
    async def get_token(self, request, client_id: str) -> Optional[Token]:
        token_record = (
            await TokenTable.query.where(TokenTable.client_id == client_id)
            .where(TokenTable.access_token == request.post.token)
            .gino.first()
        )
        if token_record is not None:
            return Token.from_orm(token_record)
